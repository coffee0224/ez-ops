...
BACKENDS = list_backends("softmax")
print(BACKENDS)
WARMUP = 10
N_REPEAT = 2000
N_TRIALS = 3
...
def _run_workload(batch, dim, label, profile, backends):
    ...
    """
    print(f"\n{'=' * 60}")
    print(f"  Softmax workload: {label}  (batch={batch}, dim={dim})")
...
def _parse_args():
    parser = argparse.ArgumentParser(add_help=False, description="Softmax kernel benchmark")
